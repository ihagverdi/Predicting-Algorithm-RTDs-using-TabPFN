"""
This module implements a simple neural network (DistNet) [2] to predict parameters of a 
log-normal distribution (shape and scale) or normal distribution (mu and sigma) 
depending on the target scaling.

References
----------
[2] Eggensperger, K., Lindauer, M., & Hutter, F. (2018).
...
def loss_fn_max(y_true, y_pred):
    ...
    assert len(y_true) == len(y_pred), (
        "loss_fn: y_true and y_pred must have the same batch size"
    )
...
def loss_fn_log(y_true, y_pred):
    ...
    assert y_pred.size(1) == 2, "y_pred must be of shape [batch, 2]"
    assert len(y_true) == len(y_pred), (
        "loss_fn_log: y_true and y_pred must have the same batch size"
    )
...
class DistNetModel:
    def __init__(
        ...
        if self.validation_available:
            ...
            self.X_valid = torch.as_tensor(
                X_valid, dtype=torch.float32, device=self.device
            )
            self.y_valid = torch.as_tensor(
        ...
        self.optimizer = optim.SGD(
            self.model.parameters(), lr=initial_lr, momentum=0.9, weight_decay=1e-4
        )
    ...
    def train(self, X_train, y_train):
        assert X_train.ndim == 2 and y_train.ndim == 2, (
            "X_train and y_train must have batch dimension"
        )
        ...
        criterion = (
            loss_fn_log if self.model_target_scale == TargetScale.LOG else loss_fn_max
        )
        ...
        for epoch in range(1, self.n_epochs + 1):
            ...
            if self.validation_available:
                ...
                if self.early_stopping:
                    ...
                    if self.epochs_no_improve >= self.early_stopping_patience:
                        print(
                            f"Early stopping | Best epoch: {epoch - self.early_stopping_patience}. Best validation loss: {self.best_val_loss:.4f}"
                        )
                        break
                self.model.train()
            ...
            if epoch == 1 or epoch % 100 == 0:
                ...
                print(
                    f"Epoch {epoch}/{self.n_epochs} | Train Loss {avg_train_loss:.4f} | Validation Loss {val_str} LR {self.scheduler.get_last_lr()[0]:.6f} | {elapsed:.1f}s"
                )
        ...
        if self.early_stopping:
            assert self.best_model_checkpoint is not None, (
                "No best model checkpoint has been found"
            )
            # Restore best model
