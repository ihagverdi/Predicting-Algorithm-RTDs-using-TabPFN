"""
This module implements the `BayesianDistNetModel`, strictly following the descriptions 
and code provided in the original paper [3].

References
----------
[3] Tuero, J. E., & Buro, M. (2021).
       "Bayes DistNet - A Robust Neural Network for Algorithm Runtime Distribution Predictions." 
       Proceedings of the AAAI Conference on Artificial Intelligence, 35(13).
"""
...
class BBBLinear2(ModuleWrapper):
    ...
    def log_prior_sum(self, x):
        first_gaussian = self.pi * self.gaussian_prior(
            x, self.prior_mu, self.prior_sigma1
        )
        second_gaussian = (1 - self.pi) * self.gaussian_prior(
            x, self.prior_mu, self.prior_sigma2
        )
...
def train_bayesianDistNet(
    ...
    if n_optim == "SGD":
        optimizer = optim.SGD(
            model.parameters(), lr=start_rate, weight_decay=0.0001, momentum=0.95
        )
    elif n_optim == "ADAM":
        optimizer = optim.Adam(
            model.parameters(), lr=start_rate, weight_decay=0.001, amsgrad=False
        )
    else:
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        print(
            "Epoch: {:>4d}, Training Loss {:>12,.4f} Training KL {:>12,.4f}, Validation Loss {:>12}, Validation log {:>12}, Validation KL {:>12}".format(
                ...
                train_kl,
                val_str,
                f"{log_loss:.4f}" if log_loss is not None else "Unavailable",
                f"{val_kl:.4f}" if val_kl is not None else "Unavailable",
            )
        )
    ...
    if early_stopping:
        assert best_model_checkpoint is not None, (
