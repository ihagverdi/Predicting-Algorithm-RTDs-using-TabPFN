'''
This module contains functions for loading and preprocessing the data from the distnet data directory. It includes functions to read the results, load features, and get the data for a specific scenario and fold. The data is preprocessed by removing instances with certain statuses (e.g., CRASHED, TIMEOUT) and optionally removing constant features. The module also handles the imputation of missing feature values.
'''
...
def load_distnet_data(
    scenario_name, fold, removeConstants=False
...
def get_data(scenario, impute_features=True):
    ...
    if impute_features:
        features = preprocess.feature_imputation(
            features, impute_val=-512, impute_with="median"
        )
    return runtimes, features, sat_ls
