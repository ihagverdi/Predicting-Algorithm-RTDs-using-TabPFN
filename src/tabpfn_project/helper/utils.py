'''
This module contains various utility functions for plotting, data processing, convenience classes, wilcoxon testing, and GPU memory / WC time tracking.
'''
...
plt.rcParams.update(
    {
        ...
        "figure.figsize": (
            6,
            4,
        ),  # Exactly matches the 6-inch textwidth from automl.sty
...
class TargetScale(Enum):
    """
    Enumeration of target variable transformation options.
    
    Defines the transformation applied to runtime targets before model training.
    
    Attributes
    ----------
    LOG : str ("log")
        Log1p transformation (log(1 + y)).
    ORIGINAL : str ("original")
        No transformation applied.
    MAX : str ("max")
        Min-max normalization by maximum value.
    
    Methods
    -------
    from_str(label)
        Convert a string (e.g., from CLI arguments) to an Enum member.
        
    Raises
    ------
    ValueError
        If the provided string does not match any valid scaling method.
    """
    LOG = "log"
    ORIGINAL = "original"
    MAX = "max"
    ...
    def from_str(cls, label: str):
        ...
        mapping = {"log": cls.LOG, "original": cls.ORIGINAL, "max": cls.MAX}
        try:
            return mapping[label.lower()]
        except KeyError:
            raise ValueError(
                f"Invalid target_scale: {label}. Expected one of {list(mapping.keys())}"
            )
...
def plot_ml_heatmap(
    ...
    if plot_metric is not None:
        requested_metrics = (
            [plot_metric] if isinstance(plot_metric, str) else plot_metric
        )
        metrics = sorted(
            requested_metrics,
            key=lambda x: (
                FIXED_METRIC_ORDER.index(x) if x in FIXED_METRIC_ORDER else 99
            ),
        )
    else:
    ...
    use_custom_grid = layout_grid is not None and (
        plot_scenario is not None or plot_metric is not None
    )
    ...
    if use_custom_grid:
        ...
        if g_rows == -1:
            g_rows = int(np.ceil(total_plots / g_cols))
        if g_cols == -1:
            g_cols = int(np.ceil(total_plots / g_rows))
    else:
        g_rows, g_cols = len(scenarios), len(metrics)

    fig, axes = plt.subplots(
        g_rows, g_cols, figsize=(8 * g_cols, 6 * g_rows), squeeze=False
    )
    ...
    for row_idx, scenario in enumerate(scenarios):
        for col_idx, metric in enumerate(metrics):
            ...
            for model_name in all_models:
                ...
                for run in runs:
                    if run["scenario"] == scenario:
                        ...
                        if ctx_size is None:
                            ctx_size = int(
                                run["train_size"][0] / run["num_samples_per_instance"]
                            )
                        ...
                        records.append(
                            {
                                ...
                                "num_samples_per_instance": run[
                                    "num_samples_per_instance"
                                ],
                                "fold": run["fold"],
                                "seed_ctx": run.get("seed_context_size", -1)
                                if run.get("seed_context_size") is not None
                                else -1,
            ...
            df_step2 = (
                df.groupby(
                    ["fold", "seed_ctx", "context_size", "num_samples_per_instance"]
                )["score"]
                .mean()
                .reset_index()
            )
            df_step3 = (
                df_step2.groupby(["fold", "context_size", "num_samples_per_instance"])[
                    "score"
                ]
                .mean()
                .reset_index()
            ...
            final_df = (
                df_step3.groupby(["context_size", "num_samples_per_instance"])["score"]
                .mean()
                .reset_index()
            )

            pivot_table = final_df.pivot(
                index="context_size", columns="num_samples_per_instance", values="score"
            )
            pivot_table = pivot_table.sort_index(ascending=True).sort_index(
...
def plot_heatmap_scatter(
    ...
    if zoom_on_model is not None and zoom_on_model not in results_data.keys():
        raise ValueError(
            f"zoom_on_model '{zoom_on_model}' not found in results_data keys."
        )
    ...
    if n_instances is not None:
        ...
        if not overlay:
            ...
            for scen in df["scenario"].unique():
                ...
                filtered_dfs.append(
                    scen_df[scen_df["context_size"] == target_instances]
                )
    ...
    if df.empty:
        raise ValueError(
            "No data found after filtering for the specified instances/samples."
        )
    ...
    fig, axes = plt.subplots(
        g_rows, g_cols, figsize=(6 * g_cols, 5 * g_rows), squeeze=False
    )
    ...
    for row_idx, scenario in enumerate(scenarios):
        ...
        for col_idx, metric in enumerate(metrics):
            ...
            for p_item in plot_items:
                ...
                df_step2 = (
                    subset.groupby(["fold", "seed_ctx", x_col])["score"]
                    .mean()
                    .reset_index()
                )
                df_step3 = (
                    df_step2.groupby(["fold", x_col])["score"].mean().reset_index()
                )
                final_stats = (
                    df_step3.groupby(x_col)["score"].agg(["mean", "std"]).reset_index()
                )
            ...
            if zoom_on_model is not None:
                ...
                for lbl, line in line_objs.items():
                    ...
                    subset = scenario_df[
                        (scenario_df["model"] == p_cfg["filter_model"])
                        & (scenario_df["metric"] == metric)
                    ]
                    if overlay:
                ...
                if out_of_bounds_models:
                    ...
                    if show_legend:
                        oob_handles = [
                            ...
                            for m in out_of_bounds_models
                        ]
                        mini_leg = ax.legend(
                            oob_handles, out_of_bounds_models, loc="lower right"
                        )
    ...
    if has_out_of_bounds and global_legend:
        special_handles["Out-of-Bounds"] = ax.scatter(
            [],
            [],
            marker="$\\gg$",
            facecolors="none",
...
def plot_feat_dropping_results(
    results_data,
    visual_config,
    log_x=False,
    log_y=False,
    ...
    if plot_scenario is not None:
        ...
        requested_scenarios = (
            [plot_scenario] if isinstance(plot_scenario, str) else plot_scenario
        )
        ...
        if missing:
            raise ValueError(
                f"Scenarios {missing} not found in data. Available: {all_available_scenarios}"
            )
    ...
    for row_idx, scenario in enumerate(scenarios):
        for col_idx, metric in enumerate(metrics):
            ...
            for model_name, (color, hatch) in visual_config.items():
                ...
                ctx_agg = (
                    subset.groupby(["n_features_keep", "fold", "seed_drop"])["value"]
                    .mean()
                    .reset_index()
                )
                ...
                fold_agg = (
                    ctx_agg.groupby(["n_features_keep", "fold"])["value"]
                    .mean()
                    .reset_index()
                )
                ...
                final_stats = (
                    fold_agg.groupby("n_features_keep")["value"]
                    .agg(["mean", "std"])
                    .reset_index()
                    .sort_values("n_features_keep")
                )
            ...
            if mark_best and abs_best_model is not None:
                ...
                line_objs[abs_best_model].set_label(
                    f"{line_objs[abs_best_model].get_label()} <-- Best"
                )
            ...
            if zoom_on_model is not None:
                ...
                for model_name, line in line_objs.items():
                    ...
                    ctx_agg = (
                        subset.groupby(["n_features_keep", "fold", "seed_drop"])[
                            "value"
                        ]
                        .mean()
                        .reset_index()
                    ...
                    stats = fold_agg.groupby("n_features_keep")["value"].agg(
                        ["mean", "std"]
                    )
...
def plot_full_instances_results(
    results_data,
    visual_config,
    log_x=False,
    log_y=False,
            ...
            for model_name, (color, hatch) in visual_config.items():
                ...
                if not seed_counts.eq(5).all():
                    raise AssertionError(
                        f"Model {model_name} in {scenario} does not have exactly 5 seeds for all x_val/folds."
                    )

                fold_means = (
                    subset.groupby(["x_val", "fold"])["value"].mean().reset_index()
                )
                final_stats = (
                    fold_means.groupby("x_val")["value"]
                    .agg(["mean", "std"])
                    .reset_index()
                    .sort_values("x_val")
                )
            ...
            if zoom_on_model is not None:
                ...
                if out_of_bounds_models:
                    ...
                    if show_legend:
                        ...
                        mini_leg = ax.legend(
                            oob_handles, out_of_bounds_models, loc="upper right"
                        )
                        mini_leg.set_zorder(10)
...
def plot_instance_level_bar_chart(
    results_data, context_size, metric_name, plot_title=None, lower_is_better=True, grid_layout=None
):
    """
    ...
    Args:
        ...
        context_size: Int or None. If None, plots all available context sizes in a grid.
        metric_name: String, the specific metric to evaluate
        plot_title: String, optional custom title (active only if context_size is not None)
        lower_is_better: Boolean, True for metrics like Loss, False for Accuracy
        grid_layout: Tuple (rows, cols), active if context_size is None. 
                     Use -1 for one dimension to auto-deduce layout size.
    ...
    # 1. Determine Context Sizes
    if context_size is None:
        contexts = set()
        for runs in results_data.values():
            for run in runs:
                contexts.add(run["context_size"])
        contexts = sorted(list(contexts))
    else:
        contexts = [context_size]

    num_plots = len(contexts)

    # 2. Grid Layout Deduction & Validation
    if context_size is None and grid_layout is not None:
        rows, cols = grid_layout
        if rows == -1 and cols == -1:
            raise ValueError("grid_layout cannot have both dimensions as -1.")
        elif rows == -1:
        ...
        if rows * cols < num_plots:
            raise ValueError(
                f"grid_layout {grid_layout} (resolved to {rows}x{cols}) "
                f"is too small for {num_plots} plots."
            )
    else:
        rows, cols = 1, 1

    # 3. Global Color Mapping Initialization (ensures consistency across plots)
    global_models = list(results_data.keys())
    max_possible_num_ranks = len(global_models)
    fallback_cmap = plt.get_cmap("Reds")
    rank_colors = {}
    
    for rank in range(1, max_possible_num_ranks + 1):
        if rank == 1:
            rank_colors[rank] = "#2CA02C"
        elif rank == 2:
            rank_colors[rank] = "#FF7F0E"
        else:
            if max_possible_num_ranks > 2:
                ratio = (rank - 3) / max(1, max_possible_num_ranks - 3)
                rank_colors[rank] = fallback_cmap(0.3 + 0.5 * ratio)
            else:
                rank_colors[rank] = "#d62728"

    # 4. Canvas Setup
    fig = plt.figure(figsize=(max(8, max_possible_num_ranks * 1.5) * cols, 6 * rows))

    # Use 2x columns in GridSpec to easily center plots that don't fill a whole row
    gs = fig.add_gridspec(rows, cols * 2)
    
    global_rank_columns = set()

    # 5. Core Loop for Each Context Size
    for idx, ctx in enumerate(contexts):
        # Determine exact position and centering pad
        row = idx // cols
        col_in_row = idx % cols
        plots_in_this_row = min(cols, num_plots - row * cols)
        
        pad = (cols * 2 - plots_in_this_row * 2) // 2
        start_col = pad + col_in_row * 2
        end_col = start_col + 2
        ...
        if not extracted_data:
            raise ValueError(
                f"No data found for context_size={ctx} and metric={metric_name}"
            )

        # --- Instance-Level Ranking ---
        all_ranks = []
        for key, model_arrays in extracted_data.items():
            ...
            ranks = df_instances.rank(
                axis=1, method="min", ascending=lower_is_better
            ).astype(int)
        ...
        # FIX: Reduced title padding from 40 to 15
        if context_size is None:
            ax.set_title(f"Context Size: {ctx}", pad=15)
        elif plot_title:
            ax.set_title(plot_title, pad=15)

        # If rendering just one individual context, place the legend on the subplot
        if context_size is not None:
            rank_patches = [
                mpatches.Patch(facecolor=rank_colors[r], edgecolor="white", label=f"Rank {r}")
                for r in sorted(rank_counts.columns)
            ]
            ax.legend(
                handles=rank_patches,
                loc="lower center",
    ...
    # 6. Global Legend Layout Formatting
    if context_size is None:
        rank_patches = [
            mpatches.Patch(facecolor=rank_colors[r], edgecolor="white", label=f"Rank {r}")
            for r in sorted(global_rank_columns)
        ]
        # FIX: Moved legend to upper center, increased font size
        fig.legend(
            handles=rank_patches,
            loc="upper center",
            bbox_to_anchor=(0.5, 0.94),
            fontsize=14,
            ncol=len(global_rank_columns),
        )
        # FIX: Compressed layout below 0.90 to safely fit suptitle + large legend up top
    ...
    for model_name, runs in results_data.items():
        ...
        for run_idx, run in enumerate(runs):
            ...
            if f_time is None or p_time is None:
                warnings.warn(
                    f"Missing time data for {model_name}, scenario '{scenario}', fold {fold}. Skipping run."
                )
                continue
            ...
            try:
                ...
                warnings.warn(
                    f"Missing score/N_test_instances data for {model_name}, scenario '{scenario}', fold {fold}. Skipping tradeoff."
                )
        ...
        for col_idx, metric in enumerate(metrics):
            ...
            for model_name, (color, hatch) in visual_config.items():
                ...
                if metric != "tradeoff":
                    # Standard time logic
                    fold_means = (
                        subset.groupby(["context_size", "fold"])["value"]
                        .mean()
                        .reset_index()
                    )
                    final_stats = (
                        fold_means.groupby("context_size")["value"]
                        .agg(["mean", "std"])
                        .reset_index()
                    )
                    ...
                    fold_means = (
                        subset.groupby(["context_size", "fold"])[
                            ["amortized_time", "score"]
                        ]
                        .mean()
                        .reset_index()
                    )
                    final_stats = (
                        fold_means.groupby("context_size")[["amortized_time", "score"]]
                        .agg(["mean", "std"])
                        .reset_index()
                    )
                    final_stats = final_stats.sort_values("context_size")
                ...
                if current_max_context < scenario_max_context:
                    ...
                    if log_x:
                        ...
                        if metric != "tradeoff":
                            ...
                            x_range = df[df["metric"] == "tradeoff"][
                                "amortized_time"
                            ].max()
                            x_offset = last_x + (x_range * 0.04)

                    sc_timeout = ax.scatter(
                        x_offset,
                        last_y,
                        color=color,
                        marker="x",
            ...
            if zoom_on_model is not None:
                ...
                for model_name, line in line_objs.items():
                    ...
                    if metric != "tradeoff":
                        ...
                        stats = fold_means.groupby("context_size")["value"].agg(
                            ["mean", "std"]
                        )
                    else:
                        fold_means = (
                            subset.groupby(["context_size", "fold"])["score"]
                            .mean()
                            .reset_index()
                        )
                        stats = fold_means.groupby("context_size")["score"].agg(
                            ["mean", "std"]
                        )
...
def plot_main_results(
    ...
    plot_scenario=None,
    plot_metric=None,
    legends_vertical=False,
    grid_layout=None,
    use_hatching=False,
    plot_transposed=False,  # <-- NEW: Added plot_transposed argument
):
    """
    ...
    Args:
        ...
        plot_transposed: Boolean, if True, columns denote scenarios and rows denote metrics.
    """
    # 1. Validation
    if set(results_data.keys()) != set(visual_config.keys()):
        raise KeyError("The keys in results_data and visual_config must match exactly.")
    ...
    if plot_metric is not None:
        if isinstance(plot_metric, str):
            if plot_metric not in all_available_metrics:
                raise ValueError(
                    f"Metric '{plot_metric}' not found in data. Available: {all_available_metrics}"
                )
            else:
                metrics = [plot_metric]
            ...
            if missing:
                raise ValueError(
                    f"Metrics {missing} not found in data. Available: {all_available_metrics}"
                )
            else:
                metrics = plot_metric
    else:
    ...
    if use_custom_grid:
        ...
        # --- MODIFIED: Adjust grid structure if transposed ---
        if plot_transposed:
            g_rows, g_cols = n_metrics, n_scenarios
        else:
            g_rows, g_cols = n_scenarios, n_metrics
    ...
    for row_idx, scenario in enumerate(scenarios):
        ...
        for col_idx, metric in enumerate(metrics):
            # --- MODIFIED: Assign subplot axes indices dynamically ---
            if use_custom_grid:
                plot_idx = row_idx * n_metrics + col_idx
                ax_row, ax_col = plot_idx // g_cols, plot_idx % g_cols
                ax = axes[ax_row][ax_col]
            ...
            # --- MODIFIED: Layout & Labels to accommodate transposition ---
            if len(scenarios) == 1 and len(metrics) == 1:
                ax.set_title(scenario)
                ax.set_ylabel(metric)
            elif use_custom_grid:
                ...
                if plot_transposed:
                    ...
                    # ax.set_ylabel(f"{metric}\n\n{scenario}" if ax_col == 0 else scenario)
                    ax.set_ylabel(f"{metric}")

                else:
                    if ax_row == 0:
                        ax.set_title(f"Metric: {metric}")
                    ax.set_ylabel(f"{scenario}\n\n{metric}" if ax_col == 0 else metric)
    ...
    for model_name, runs in results_data.items():
        for run_idx, run in enumerate(runs):
            ...
            if f_vram is None or p_vram is None:
                warnings.warn(
                    f"Missing VRAM data for {model_name}, scenario '{scenario}', fold {fold}. Skipping run."
                )
                continue
        ...
        for loop_c in loop_cols:
            ...
            for metric in metrics_to_plot:
                for model_name, (color, hatch) in visual_config.items():
                    ...
                    final_label = (
                        f"Fit VRAM (GB)" if metric == "fit" else f"Predict VRAM (GB)"
                    )
                    if not merge_plots:
                        final_label = model_name

                    (line,) = ax.plot(
                        x, y_mean, label=final_label, color=plot_color, marker="o"
                    )
            ...
            if zoom_on_model is not None:
                ...
                for final_label, line in line_objs.items():
                    ...
                    if merge_plots:
                        ...
                        model_for_subset = list(visual_config.keys())[
                            0
                    ...
                    subset = df[
                        (df["model"] == model_for_subset)
                        & (df["scenario"] == scenario)
                        & (df["metric"] == metric_for_subset)
                    ]
                    if subset.empty:
                        continue
            ...
            if merge_plots:
                ...
                metric_display_name = (
                    "Fit VRAM (GB)"
                    if metrics_to_plot[0] == "fit"
                    else "Predict VRAM (GB)"
                )
                if row_idx == 0:
...
def plot_combined_time_vram_results(
    results_data,
    visual_config,
    log_x=False,
    log_y=False,
    ...
    for model_name, runs in results_data.items():
        ...
        for run_idx, run in enumerate(runs):
            ...
            if f_time is None or p_time is None:
                warnings.warn(
                    f"Missing time data for {model_name}, scenario '{scenario}', fold {fold}."
                )
            if (f_vram is None or p_vram is None) and is_tabpfn:
                warnings.warn(
                    f"Missing VRAM data for {model_name}, scenario '{scenario}', fold {fold}."
                )
            ...
            try:
                ...
                amortized_time = (
                    (f_time + p_time) / N
                    if (f_time is not None and p_time is not None)
                    else None
                )
        ...
        for col_idx, col_metric in enumerate(metrics_cols):
            ...
            if col_metric != "vram":
                for model_name, (color, hatch) in visual_config.items():
                    ...
                    if col_metric != "tradeoff":
                        ...
                        final_stats = (
                            fold_means.groupby("context_size")[
                                ["amortized_time", "score"]
                            ]
                            .agg(["mean", "std"])
                            .reset_index()
                    ...
                    (line,) = ax.plot(
                        x, y_mean, label=model_name, color=color, marker="o"
                    )
                    line_objs[model_name] = line
                    ...
                    if zoom_on_model == model_name:
                        zoom_bounds["min"] = min(
                            zoom_bounds["min"], (y_mean - y_std).min()
                        )
                        zoom_bounds["max"] = max(
                            zoom_bounds["max"], (y_mean + y_std).max()
                        )
            ...
            # -------------------------------------------------------------
                ...
                for model_name, (color, hatch) in visual_config.items():
                    ...
                    plot_color = (
                        color  # <-- NEW: Uses the model's actual dictionary color
                    )
                    hatch_val = hatch if use_hatching else None
                    final_label = (
                        "TabPFN (Fit + Predict VRAM in GB)"  # <-- NEW: Unified label
                    )
            ...
            if zoom_on_model is not None:
                ...
                for lbl, line in line_objs.items():
                    if col_metric != "vram":
                        ...
                        model_to_check = next(
                            (m for m in visual_config.keys() if "tabpfn" in m.lower()),
                            list(visual_config.keys())[0],
                        )

                        subset = df[
                            (df["model"] == model_to_check)
                            & (df["scenario"] == scenario)
                            & (df["metric"] == vram_metric)
                        ]
                        if subset.empty:
                            continue
            ...
            if col_metric == "fit_time":
                ...
                ax_title, y_label, x_label = (
                    "Tradeoff (Score vs Time)",
                    "Score (NLLH)",
                    "Amortized Time (s/pred)",
                )
            else:  # vram
...
def plot_rank_over_context_size(
    results_data,
    visual_config,
    metric_name,
    plot_title=None,
    ...
    """
    Plots a line chart showing models' global instance-level rankings,
    with shaded bands representing cross-fold variance.
    ...
    Args:
        ...
        bounds: Tuple (lower_b, upper_b) to filter the plotted context sizes
        show_variance: Boolean, whether to shade the variance
        shade_type: String, "std" for Standard Deviation, "SE" for 95% Confidence Interval
    """
    if not results_data:
        raise ValueError("results_data is empty.")
    ...
    for ctx_size in all_ctx_sizes:
        ...
        ctx_ranks = []
        for split_key, model_arrays in extracted_data.items():
            available_models = list(model_arrays.keys())
            ...
            for m in available_models:
                for r in ranks[m]:
                    ctx_ranks.append({"model": m, "split_key": split_key, "rank": r})

        df_ctx_ranks = pd.DataFrame(ctx_ranks)
        if df_ctx_ranks.empty:
            continue

        global_stats = df_ctx_ranks.groupby("model")["rank"].mean().rename("global_mean")
        
        split_means = df_ctx_ranks.groupby(["model", "split_key"])["rank"].mean()
        fold_stats = split_means.groupby("model").agg(["std", "count"]).rename(
            columns={"std": "fold_std", "count": "fold_count"}
        ).fillna(0)
        
        combined_stats = pd.concat([global_stats, fold_stats], axis=1)
        max_possible_rank = df_ctx_ranks["model"].nunique()

        for m, row in combined_stats.iterrows():
            all_records.append(
                {
                    "model": m, 
                    "context_size": ctx_size, 
                    "mean_rank": row["global_mean"],
                    "std_rank": row["fold_std"],
    ...
    if df_plot.empty:
        raise ValueError(
            f"No valid rank data could be computed for metric '{metric_name}'."
        )

    # --- Pre-Plot Data Filtering ---
    lower_b, upper_b = bounds
    
    model_true_max_ctx = df_plot.groupby("model")["context_size"].max().to_dict()

    if lower_b is not None:
        df_plot = df_plot[df_plot["context_size"] >= lower_b]
        all_ctx_sizes = [c for c in all_ctx_sizes if c >= lower_b]
        
    if upper_b is not None:
        df_plot = df_plot[df_plot["context_size"] <= upper_b]
        all_ctx_sizes = [c for c in all_ctx_sizes if c <= upper_b]

    if df_plot.empty:
        raise ValueError("No valid rank data remains after applying bounds.")
    # ------------------------------------
    ...
    for model_name, (color, hatch) in visual_config.items():
        ...
        y = subset["mean_rank"]
        y_std = subset["std_rank"]
        n_count = subset["count"]
        max_rank = subset["max_rank"]

        ax.plot(
            x, y, label=model_name, color=color, marker="o", markersize=6, linewidth=2.5, zorder=4
        )

        # --- Shading for Cross-Fold Variance / SE ---
        if show_variance:
            is_se = shade_type.upper() == "SE"
            
            if is_se:
                # 95% Confidence Interval across the folds
                margin = 1.96 * (y_std / np.sqrt(n_count))
                fill_alpha = 0.3
                edge_kw = {"edgecolor": color, "linewidth": 1.0, "alpha": 0.5}
            else:
            ...
            lower_bound = np.maximum(1.0, y - margin)
            upper_bound = np.minimum(max_rank, y + margin)
        ...
        # --- Early Termination Marker ---
        true_max_x = model_true_max_ctx.get(model_name)
        terminated_early = true_max_x < max_global_ctx
        
        termination_in_bounds = True
        if lower_b is not None and true_max_x < lower_b:
            termination_in_bounds = False
        if upper_b is not None and true_max_x >= upper_b:
            termination_in_bounds = False

        if terminated_early and termination_in_bounds:
            last_x = x.iloc[-1]
            last_y = y.iloc[-1]
            ...
            if log_x:
                ...
                x_range = all_ctx_sizes[-1] - all_ctx_sizes[0]
                x_range = x_range if x_range > 0 else last_x
                x_offset = last_x + (x_range * 0.04)
    ...
    if plot_title and plot_scenario:
...
def sample_k_per_instance(X, y, ids, k, seed):
    """
    Randomly sample up to k samples per unique instance without replacement.
    
    Groups data by instance ID, randomly selects up to k samples from each group,
    and returns the subsampled data in original order.
    
    Parameters
    ----------
    X : np.ndarray
        Flattened features of shape (M, D).
    y : np.ndarray
        Flattened targets of shape (M, 1).
    ids : np.ndarray
        Flattened instance IDs of shape (M,).
    k : int
        Maximum number of samples to keep per unique instance.
    seed : int
    ...
    -------
    tuple
        (X_sampled, y_sampled, ids_sampled) — subsampled arrays with preserved structure.
    """
    ...
    group_offsets = np.repeat(
        start_indices, np.diff(np.append(start_indices, len(ids)))
    )
...
def generate_experiment_id(cfg) -> str:
    ...
    params = [
        ...
        (
            "drop",
            f"{cfg.feature_drop_rate}_keep{cfg.n_features_keep}_s{cfg.seed_feature_drop_rate}",
        ),
...
def find_optimal_context(
    results_list, scenario, target_metric, alpha=0.05, display_results=True
):
    """
    ...
    if not records:
        raise ValueError(
            f"No data found for scenario '{scenario}'. Please check the name."
        )
    ...
    summary_df = (
        fold_df.groupby("context_size")["score"].agg(["mean", "std"]).reset_index()
    )
    ...
    scores_best = (
        fold_df[fold_df["context_size"] == c_best].sort_values("fold")["score"].values
    )
    ...
    for _, row in summary_df.iterrows():
        ...
        scores_test = (
            fold_df[fold_df["context_size"] == c_test]
            ...
            try:
                ...
                with warnings.catch_warnings():
                    ...
                    stat, p_value = wilcoxon(
                        scores_test, scores_best, alternative="greater"
                    )
            except ValueError:
...
def subsample_data(*arrays, context_size, seed, with_replacement=True):
    """
    Synchronously subsamples one or more arrays along their first dimension.

    Args:
        *arrays (np.ndarray): One or more arrays to subsample. All must have the same length `N` along the first dimension (e.g., shape `(N, ...)`).
        context_size (int): The number of samples `M` to draw.
        seed (int): Random seed for reproducibility.
        with_replacement (bool, optional): Whether to sample with replacement. Defaults to True.

    Returns:
        Tuple[np.ndarray, ...]: A tuple of the subsampled arrays, each with shape `(M, ...)`, preserving their original trailing dimensions.
    """
    if not arrays:
    ...
    selected_indices = rng.choice(
        n_samples, size=context_size, replace=with_replacement
    )
...
def append_random_columns(
    *arrays,
    ...
    N = arrays[0].shape[0]
    D = arrays[0].shape[1]
    assert all(arr.shape[1] == D for arr in arrays), (
    ...
    augmented_arrays = tuple(
        np.hstack(
            (
                arr,
                rng.standard_normal(
                    size=(arr.shape[0], n_random_cols), dtype=np.float64
                ),
            )
        )
        for arr in arrays
...
def subsample_features(
    ...
    *arrays,
    ...
    if size_features > n_features:
        raise ValueError(
            f"n_features_keep ({size_features}) cannot be greater than total features ({n_features})"
        )
...
def fetch_save_dict(
    ...
    if search_key is None:
        ...
        output_name = (
            f"{model_file_name}_{search_key}_{search_value}{scenario_label}.pkl"
        )
