'''
This module contains the `prepare_datasets` function, which is responsible for preparing the training and testing datasets based on the provided experiment configuration.
'''
...
def prepare_datasets(
    ...
    """Prepare datasets applying structural adjustments based on the provided configuration.
    ...
    tuple
        ...
        - X_train_flat : np.ndarray, shape (M*K, D')
          Flattened training features, where M is number of training instances, K is number of samples per instance, D' is feature count.
        - X_test : np.ndarray, shape (N_test, D')
          Test features.
        - y_train_flat : np.ndarray, shape (M*K, 1)
          Flattened training targets.
        - y_test : np.ndarray, shape (N_test, K)
          Test targets (multiple samples per test instance).
        - train_group_ids_flat : np.ndarray, shape (M*K,)
          Flattened training instance IDs indicating which instance each sample belongs to.
    """
    do_feature_drop = (
        cfg.feature_drop_rate is not None or cfg.n_features_keep is not None
    )
    X_train, X_test, y_train, y_test = load_distnet_data(
        cfg.scenario, cfg.fold, removeConstants=do_feature_drop
    )
    ...
    if cfg.num_samples_per_instance != 100:
        assert 1 <= cfg.num_samples_per_instance <= 100, (
            f"num_samples_per_instance must be between 1 and 100, got {cfg.num_samples_per_instance}"
        )
        assert cfg.seed_samples_per_instance is not None
        if cfg.num_samples_per_instance < 100:
            y_train = subsample_targets_per_instance(
                y_train, cfg.num_samples_per_instance, cfg.seed_samples_per_instance
            )
    ...
    train_group_ids_flat = np.repeat(
        train_group_ids, repeats=cfg.num_samples_per_instance
    )
    ...
    if do_feature_drop:
        ...
        assert cfg.seed_feature_drop_rate is not None, (
            "Seed for feature dropping must be provided."
        )
