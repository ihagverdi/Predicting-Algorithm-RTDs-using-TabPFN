'''
This module defines the `BaseModelHandler` class and its subclasses for different models (DistNet, BayesianDistNet, TabPFN, Lognormal, Random Forest (RF), Gaussian Process (GP)). Each handler implements the `run` method to train the respective model, make predictions, and calculate evaluation metrics based on the provided training and test data. The handlers also manage model-specific configurations, preprocessing steps, and resource tracking (e.g., GPU memory and time).
'''
...
from tabpfn_project.globals import LLH_EPSILON, N_GRID_POINTS, RANDOM_STATE
from tabpfn_project.helper.utils import (
    TargetScale,
    generate_experiment_id,
    sample_k_per_instance,
    track_gpu_memory_and_time,
)
...
class BaseModelHandler:
    def run(
        ...
        y_test: np.ndarray,
        train_group_ids_flat: np.ndarray,
    ) -> Dict:
            """
            Train a model and compute predictions and evaluation metrics.
            
            Subclasses must implement this method to train their respective models,
            ...
            X_train_flat : np.ndarray
                Flattened training features.
            X_test : np.ndarray
                Test features.
            y_train_flat : np.ndarray
                Flattened training targets.
            y_test : np.ndarray
                Test targets.
            train_group_ids_flat : np.ndarray
                Flattened training group IDs (instance indices).
            
            Returns
            -------
            dict
                Dictionary containing:
                - "y_test_preds": Predictions on test data.
                - "result_metrics": Dictionary with "metrics_summary" and "instance_summary".
                - "model_specific_info": Dictionary with model-specific metadata.
...
class DistNetHandler(BaseModelHandler):
    def run(
        ...
        assert cfg.target_scale.value in ["max", "log"], (
            "Invalid target_scale for DistNet. Only 'max' and 'log' are supported."
        )
        device = torch.device(
        ...
        if cfg.early_stopping:
            ...
            tr_idx, val_idx = next(
                gss.split(X_train_flat, y_train_flat, groups=train_group_ids_flat)
            )

            X_tr, X_val = X_train_flat[tr_idx], X_train_flat[val_idx]
            y_tr, y_val = y_train_flat[tr_idx], y_train_flat[val_idx]
            ids_tr, ids_val = (
                train_group_ids_flat[tr_idx],
                train_group_ids_flat[val_idx],
            )
            ...
            model = DistNetModel(
                model_target_scale=cfg.target_scale,
                n_input_features=X_tr.shape[1],
                n_epochs=DISTNET_N_EPOCHS,
                batch_size=DISTNET_BATCH_SIZE,
                wc_time_limit=DISTNET_WCT,
                X_valid=X_val,
                y_valid=y_val_scaled,
                early_stopping=True,
                early_stopping_patience=DISTNET_ES_PATIENCE,
                random_state=RANDOM_STATE,
            )
            X_train_flat, y_train_scaled, train_group_ids_flat = (
                X_tr,
                y_tr_scaled,
                ids_tr,
            )
        ...
        return {
            ...
            "model_specific_info": {
                ...
                "train_group_ids": train_group_ids_flat,
                "n_samples_val": X_val.shape[0] if X_val is not None else None,
                "n_features_val": X_val.shape[1] if X_val is not None else None,
            },
        }
...
class BayesianDistNetHandler(BaseModelHandler):
    def run(
        ...
        assert cfg.target_scale == TargetScale.MAX, (
            "BayesianDistNet currently only supports 'max' target scale."
        )
        device = torch.device(
            "cuda" if (torch.cuda.is_available() and not cfg.use_cpu) else "cpu"
        )
        ...
        if cfg.early_stopping:
            ...
            handler = BayesianDistNetModel(
                n_input_features=X_tr.shape[1], device=device, early_stopping=False
            )
        ...
        print(
            f"BayesianDistNet Metrics Summary scale={cfg.target_scale.value}: {metrics_sum}"
        )
...
class TabPFNHandler(BaseModelHandler):
    def run(
        ...
        from tabpfn_project.helper.tabpfn_helpers import (
            batch_predict_tabpfn,
            oracle_predict_tabpfn,
        )
        ...
        if not cfg.oracle:
            if cfg.jitter_x:
                assert cfg.jitter_val is not None, (
                    "jitter_val must be provided when jitter_x is enabled."
                )
                print(
                    f"Applying baseline feature jitter with intensity {cfg.jitter_val} to training data."
                )
                X_train_flat = add_feature_jitter(
                    X_train_flat,
                    jitter_intensity=cfg.jitter_val,
                    random_state=RANDOM_STATE,
                )

            if cfg.rand_extend_x:
                assert cfg.n_rand_cols is not None, (
                    "n_rand_cols must be provided when rand_extend_x is enabled."
                )
                print(
                    f"Applying random feature extension with {cfg.n_rand_cols} columns."
                )
                X_train_flat, X_test = append_random_columns(
                    X_train_flat,
                    X_test,
                    n_random_cols=cfg.n_rand_cols,
                    random_state=RANDOM_STATE,
                )
        ...
        if cfg.oracle:
            ...
            with track_gpu_memory_and_time(device) as stats:
                preds, y_scaler = oracle_predict_tabpfn(
                    model, y_test, target_scale=cfg.target_scale
                )
            all_preds = [preds]
            ...
            if cfg.remove_duplicates:
                print(
                    f"Removing duplicates: keeping 1 sample per instance in training data to prevent leakage."
                )
                X_train_flat, y_train_flat, train_group_ids_flat = (
                    sample_k_per_instance(
                        X_train_flat,
                        y_train_flat,
                        train_group_ids_flat,
                        k=1,
                        seed=RANDOM_STATE,
                    )
                )
            ...
            with track_gpu_memory_and_time(device) as stats:
                preds = batch_predict_tabpfn(
                    model, X_test, validation_batch_size=TABPFN_VAL_BATCH_SIZE
                )
            all_preds = [preds]
            mem_stats["predict"] = stats
...
class LognormalHandler(BaseModelHandler):
    def run(
        ...
        if cfg.oracle:
            ...
            mu, sigma = (
                torch.mean(log_y, dim=1, keepdim=True),
                torch.std(log_y, dim=1, keepdim=True),
            )
        else:
        ...
        metrics_sum, inst_sum = calculate_metrics_lognormal(
            y_test, dist, device=device, N_grid_points=N_GRID_POINTS
        )
...
class RFHandler(BaseModelHandler):
    def run(
        ...
        from tabpfn_project.helper.calculate_metrics import (
            calculate_metrics_random_forest,
        )
        ...
        device = torch.device(
            "cuda"
            if (torch.cuda.is_available() and not getattr(cfg, "use_cpu", False))
            else "cpu"
        )
        ...
        if cfg.do_hpo:
            ...
            cs.add(
                [
                    ...
                    Float(
                        "min_impurity_decrease", (1e-10, 1e-3), log=True, default=1e-10
                    ),
            ...
            def smac_objective(config, seed) -> float:
                ...
                if not bootstrap_val:
                    max_samples_val = (
                        None  # Ensure max_samples is ignored when bootstrap=False
                    )

                gkf = GroupKFold(n_splits=3)
                metrics_per_fold = []
                for in_train_idx, in_val_idx in gkf.split(
                    X_train_flat, y_train_flat, groups=train_group_ids_flat
                ):
                    ...
                    z_in_val = torch.log1p(
                        torch.as_tensor(y_in_val, dtype=torch.float32, device=device)
                    )
            ...
            smac = HyperparameterOptimizationFacade(
                scenario=scenario, target_function=smac_objective, overwrite=False
            )
            ...
            print(
                f"HPO Trials - Submitted: {num_submitted_trials}, Finished: {num_finished_trials}, Unique Configs Evaluated: {num_unique_configs}"
            )
...
class GPHandler(BaseModelHandler):
    def run(
        ...
        assert cfg.target_scale == TargetScale.LOG, (
            "GPHandler currently only supports 'log' target scale."
        )
        ...
        kernel = ConstantKernel(1.0, (1e-5, 1e5)) * Matern(
            length_scale=1.0, length_scale_bounds=(1e-5, 1e5), nu=1.5
        ...
        print(
            f"Fitting Exact GP on {X_train_scaled.shape[0]} samples. This may take a while..."
        )
        ...
        metrics_sum, inst_sum = calculate_metrics_gp(
            y_test,
            (y_pred_log_mean, y_pred_log_std),
